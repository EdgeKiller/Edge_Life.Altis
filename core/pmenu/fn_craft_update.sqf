...
//_str = [_x select 0] call life_fnc_varToStr; Getting craft from an other file for a coming update

/*
switch (true) do
{
	case (_item == "hgun_P07_F"):
	{
		_str = ["life_inv_goldbar"] call life_fnc_varToStr;
		_struct = _struct + format["5x %1<br/>",_str];
		_str = ["life_inv_copperr"] call life_fnc_varToStr;
		_struct = _struct + format["3x %1<br/>",_str];
		_str = ["life_inv_ironr"] call life_fnc_varToStr;
		_struct = _struct + format["2x %1<br/>",_str];
		//hint format["%1",_str];
	};
};*/
